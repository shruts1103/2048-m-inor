...
const colorPalette = {
    5: 60, 10: 50, 15: 40, 20: 30, 32: 20, 64: 10, 128: 70, 256: 80, 512: 90, 1024: 0, 2028: 100
}
...
function initializeBoard(board) {
    for (let i=0; i<2; i++) {
        ...
        row = emptyCell[0]
        col = emptyCell[1]
        board[row][col] = [5, 10][Math.floor(Math.random()*2)]
    }
    return board
}
...
function generateAnotherNumInRandomEmptyCell(board) {
    let randomNumberFromAnArray = [5, 10][Math.floor(Math.random()*2)]
    let emptyCells = getEmptyCells(board)
